...
C_LANGUAGE = Language('./build/my-languages.so', 'cpp')
parser = Parser()
...
def analyze_if(node, code):
    ...
    # if_node.feature = encode_ast_node_text(if_node.code, tokenizer, 32)  # 使用新函数
    return if_node
...
def analyze_while(node, code):
    ...
    # while_node.feature = encode_ast_node_text(while_node.code, tokenizer, 32)  # 使用新函数
    return while_node
...
def analyze_do_while(node, code):
    ...
    while_node.code = index_to_code_token((while_node.start_point, while_node.end_point), code)
    # do_node.feature = encode_ast_node_text(do_node.code, tokenizer, 32)  # 使用新函数
    return do_node
...
def analyze_for(node, code):
    ...
    # condition.feature = encode_ast_node_text(condition.code, tokenizer, 32)  # 使用新函数
    return condition
...
def encode_ast_node_text(text: str, tokenizer, feature_dim: int):
    tokenized_text = tokenizer.encode(text, add_special_tokens=True, max_length=feature_dim, padding='max_length',
                                      truncation=True)
...
def create_node_features(df_path):
    ...
    # 遍历df_path列表
    for i in range(n):
        node_features.append(df_path[i][0])
